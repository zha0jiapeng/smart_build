 ...
 * 处理第二种目录格式的类。
 */
public class FormatTwoProcessor {
    public void processFiles(FTPClient ftpClient, String remoteDir) throws IOException {
        ...
        for (FTPFile file : files) {
            if (file.isDirectory()) {
                System.out.println("[Format 2 Directory] " + file.getName()); // 输出目录信息
            } else {
                System.out.println("[Format 2 File] " + file.getName() + " (Size: " + file.getSize() + " bytes)"); // 输出文件信息
            }
        }
    }
}